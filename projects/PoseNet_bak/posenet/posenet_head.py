...
if new_method:
    ...
    def keypoint_rcnn_loss(pred_keypoint_logits, instances, normalizer):
        ...
        for instances_per_image in instances:
            if len(instances_per_image) == 0:
                continue
            gt_kps = instances_per_image.gt_keypoints.tensor
            gt_boxes = instances_per_image.proposal_boxes.tensor
            gt_whs = gt_boxes[:,[2,3]] - gt_boxes[:,[0,1]]
            gt_kps[:,:,[0,1]] = gt_kps[:,:,[0,1]] / gt_whs.unsqueeze(1).repeat(1, gt_kps.shape[1], 1)
            gt_keypoints.append(gt_kps)
    ...
    def keypoint_rcnn_inference(pred_keypoint_logits, pred_instances):
        ...
        keypoint_results = pred_keypoint_logits.split(num_instances_per_image, dim=0)
        for keypoint_results_per_image, instances_per_image in zip(keypoint_results, pred_instances):
            # keypoint_results_per_image is (num instances)x(num keypoints)x(x, y, score)
            boxes = instances_per_image.pred_boxes.tensor
            whs = boxes[:,[2,3]] - boxes[:,[0,1]]
            keypoint_results_per_image[:,:,[0,1]] = keypoint_results_per_image[:,:,[0,1]] * whs.unsqueeze(1).repeat(1, keypoint_results_per_image.shape[1], 1)
            keypoint_results_per_image[:,:,2] = torch.sigmoid(keypoint_results_per_image[:,:,2])
            instances_per_image.pred_keypoints = keypoint_results_per_image

    @ROI_KEYPOINT_HEAD_REGISTRY.register()
    class PoseConvHead(nn.Module):
        """
        ...
        def __init__(self, cfg, input_shape: ShapeSpec):
            ...
            """
            super(PoseConvHead, self).__init__()

            # fmt: off
            conv_dims = cfg.MODEL.ROI_KEYPOINT_HEAD.CONV_DIMS
            num_fc = cfg.MODEL.POSENET_KPFC.NUM_FC
            fc_dim = cfg.MODEL.POSENET_KPFC.FC_DIM
            self.num_keypoints = cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
            in_channels = input_shape.channels
            # fmt: on
            tower = []
            for layer_channels in conv_dims:
                tower.append(nn.Conv2d(in_channels, layer_channels, kernel_size=3, stride=1, padding=1, bias=True))
                tower.append(nn.GroupNorm(32, layer_channels))
                tower.append(nn.ReLU())
                in_channels = layer_channels
            self.add_module('tower', nn.Sequential(*tower) if tower is not None else None)
            output_size = in_channels * input_shape.height * input_shape.width
            fc = []
            for _ in range(num_fc):
                fc.append(nn.Linear(output_size, fc_dim))
                fc.append(nn.ReLU())
                output_size = fc_dim
            self.add_module('fc', nn.Sequential(*fc) if fc is not None else None)
            self.logits = nn.Linear(output_size, self.num_keypoints * 3)
            for modules in [self.tower, self.fc]:
